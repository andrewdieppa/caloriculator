...
import { toggleFatModal } from '../../store/uiSlice';
import { setFatPerc } from '../../store/mealsSlice';

const FatModal = () => {
  const dispatch = useDispatch();
  const { meals, numMeals, fatPercTotal } = useSelector(
    state => state.mealsData
  );
  const { showFatModal } = useSelector(state => state.ui);
  ...
  return (
    <Modal open={showFatModal} onClose={() => dispatch(toggleFatModal())}>
      <>
        ...
        <Box sx={style}>
              ...
              {meals.map(meal => {
                return (
                  <MealMacPercSlider
                    ...
                    macroPercTotal={meal.fatPercTotal}
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Box sx={{ mr: 1 }}>
              <ValidIndicator validityVar={fatPercTotal} />
            </Box>
